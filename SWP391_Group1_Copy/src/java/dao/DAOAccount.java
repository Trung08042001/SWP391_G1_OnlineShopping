...
public class DAOAccount {
    ...
    public void UpdateDataAccount(String fullname, boolean gender, String address, String email, String password,
        ...
        String sql = "UPDATE onlineshopping.account \n"
                + "SET fullname=?, gender=?, address=?, email=?, password=?, phone_number=?, roleID=?, status=?, update_at=? \n"
                + "WHERE id=?; ";
        try ( Connection conn = new DBContext().getConnection();  PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setString(7, roleID);
            ps.setString(8, status);
            ps.setTimestamp(9, new Timestamp(System.currentTimeMillis())); // Set current timestamp
            ps.setString(10, id);
            ps.executeUpdate();
