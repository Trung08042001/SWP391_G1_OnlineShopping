 ...
 * @author Nitro
 */
public class ShipperUndeliveredOrder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ShipperUndeliveredOrder</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ShipperUndeliveredOrder at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int count2 = od.getTotalShipperUnDelivered(shipperID);
            int endPage = count2 / 10;
            if (count2 % 10 != 0) {
                endPage++;
            }
            ...
            request.setAttribute("totalOrderShipperUnDeliveredOrder", count2);

            request.setAttribute("endP", endPage);
            request.setAttribute("indexPage", indexPage);  // hiện trang in màu trag đấy

            Shipper s = dp.getShipperByID(shipperID);
            List<Order> listO = od.ShipperUnDeliveredOrder(s.getShipperID(), index);
            request.setAttribute("listO", listO);
            request.getRequestDispatcher("/view/shipperOrder/shipperUndeliveredOrder.jsp").forward(request, response);
        } catch (Exception e) {
