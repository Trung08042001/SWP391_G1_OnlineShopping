 ...
 * @author Nitro
 */
public class ManageConfirmOrderDetail extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ConfirmOrderDetail</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ConfirmOrderDetail at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (o != null && o.getStatus() == 1) {
                ...
                List<OrderDetail> listO = od.ConfirmOrderDetail(orderID);
                Order odr = od.getOrderByOIDStatus(orderID);
                request.setAttribute("data", odr);
                request.setAttribute("listO", listO);
                request.getRequestDispatcher("/view/managerOrder/confirmOrderDetail.jsp").forward(request, response);
